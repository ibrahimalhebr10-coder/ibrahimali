/*
  # إصلاح دالة get_farm_path_payments_detail - استخدام LEFT JOIN

  ## المشكلة
  
  الدالة كانت تستخدم INNER JOIN مع جدول user_profiles،
  مما يؤدي إلى استبعاد السجلات التي لا يوجد لها ملف تعريف مستخدم.

  ## الحل
  
  استخدام LEFT JOIN لعرض جميع الدفعات حتى لو لم يكن هناك ملف تعريف مستخدم،
  مع توفير قيمة افتراضية للاسم.
*/
...
BEGIN
  ...
  SELECT 
    mp.id as payment_id,
    mp.user_id,
    COALESCE(up.full_name, 'غير معروف') as full_name,
    mp.tree_count,
    mp.amount_due,
