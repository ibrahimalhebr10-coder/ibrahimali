/*
  # توحيد دالة جلب صيانات العملاء - الحل الجذري
  
  ## المشكلة
  - كان لدينا دالتين منفصلتين للمسار الزراعي والاستثماري
  - الدالة القديمة get_client_maintenance_records تجلب من investment_assets فقط
  - الدالة الجديدة get_agricultural_client_maintenance_records تجلب من reservations فقط
  - هذا معقد وغير جذري
  
  ## الحل الجذري
  - دالة واحدة موحدة تعمل للمسارين
  - تستقبل parameter: path_type ('agricultural' أو 'investment')
  - تجلب من الجدول المناسب حسب المسار
  - كود موحد وبسيط
  
  ## البنية الموحدة
  - المسار الزراعي: reservations (path_type = 'agricultural')
  - المسار الاستثماري: investment_assets
*/
...
DROP FUNCTION IF EXISTS get_agricultural_client_maintenance_records(uuid);

-- =====================================================
-- 2. دالة موحدة للمسارين
-- =====================================================
...
BEGIN
  ...
  ELSE
    ...
    FROM maintenance_records mr
    JOIN farms f ON f.id = mr.farm_id
    JOIN investment_assets ia ON ia.farm_id = mr.farm_id AND ia.user_id = client_user_id
    LEFT JOIN maintenance_fees mf ON mf.maintenance_id = mr.id
    LEFT JOIN maintenance_payments mp ON mp.maintenance_fee_id = mf.id AND mp.user_id = client_user_id
    WHERE mr.status = 'published'
    GROUP BY 
