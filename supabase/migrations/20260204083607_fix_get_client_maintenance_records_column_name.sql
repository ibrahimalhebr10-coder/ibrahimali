/*
  # إصلاح دالة get_client_maintenance_records - اسم العمود الصحيح

  ## المشكلة
  - الدالة تستخدم `r.tree_count` لكن العمود الحقيقي اسمه `total_trees`
  - هذا يسبب خطأ: column r.tree_count does not exist

  ## الحل
  - تصحيح الدالة لاستخدام `r.total_trees` بدلاً من `r.tree_count`
*/
...
-- =====================================================
-- 2. دالة مصححة مع اسم العمود الصحيح
-- =====================================================
...
COMMENT ON FUNCTION get_client_maintenance_records IS 'دالة موحدة لجلب صيانات العملاء للمسارين الزراعي والاستثماري - مصححة';
