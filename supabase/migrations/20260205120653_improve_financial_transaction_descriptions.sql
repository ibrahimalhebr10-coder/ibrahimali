/*
  # تحسين أوصاف العمليات المالية

  1. التغييرات
    - تحسين وصف الإيراد عند سداد رسوم الصيانة
    - إضافة اسم المسار بشكل واضح (أشجاري الخضراء / أشجاري الذهبية)
    
  2. الهدف
    - جعل سجل العمليات أكثر وضوحاً
    - تسهيل قراءة السجلات المالية مستقبلاً
    - لا تغيير في المنطق أو الحسابات
*/

-- تحديث دالة إتمام الدفع مع وصف محسّن
...
BEGIN
  ...
  IF p_payment_method IS NOT NULL AND p_payment_method != 'bank_transfer' THEN
    INSERT INTO farm_financial_transactions (
      ...
      'رسوم صيانة – ' || v_path_name || ' (سداد إلكتروني)',
      now(),
      v_user_id
    )
...
- السداد الإلكتروني: ينشئ إيراد تلقائياً بوصف واضح
- الوصف يتضمن اسم المسار (أشجاري الخضراء / الذهبية)
