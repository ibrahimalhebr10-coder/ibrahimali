/*
  # إصلاح RPC function للتحقق من primary_identity
  
  1. التعديلات
    - تحديث `check_account_type` للتحقق من `primary_identity` في `user_profiles`
    - إذا كان primary = 'agricultural' → نرجع 'farmer'
    - إذا كان primary = 'investment' → نرجع 'investor'
    - نتحقق من reservations
    - نتحقق من influencer_partners
  
  2. الأمان
    - Function تستخدم `auth.uid()` للتحقق من المستخدم الحالي
    - تقرأ فقط بيانات المستخدم الحالي
*/

-- حذف النسخة القديمة
DROP FUNCTION IF EXISTS check_account_type();

-- إنشاء النسخة الجديدة المحسّنة
